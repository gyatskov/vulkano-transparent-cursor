// This version is written using dynamic rendering instead of render pass
...
fn main() -> Result<(), impl Error> {
    ...
    let instance_wrapped = Instance::new(
        library,
        InstanceCreateInfo {
    ...
    let instance = instance_wrapped.unwrap();

    println!("Unwrapped Instance: {}", type_to_str(&instance.as_ref()));
    ...
    let (mut swapchain, images) = {
        ...
        Swapchain::new(
            ...
            SwapchainCreateInfo {
                ...
                // Use transparency through alpha channel if possible
                composite_alpha: if surface_capabilities
                    .supported_composite_alpha
                    .contains_enum(CompositeAlpha::PostMultiplied)
                {
        ...
        GraphicsPipeline::new(
            ...
            GraphicsPipelineCreateInfo {
                ...
                input_assembly_state: Some(InputAssemblyState {
                    topology: input_assembly::PrimitiveTopology::TriangleStrip,
                    ..Default::default()
                }),
    ...
    // Allocator for descriptor sets (e.g. uniform buffers)
    ...
    event_loop.run(move |event, elwt| {
        ...
        match event {
            ...
            Event::WindowEvent {
                ...
                // TODO: Track DPI changes using `WindowEvent::ScaleFactorChanged`
                let position_physical = position.cast::<f32>();
                let _position_logical = position.to_logical::<f32>(window.scale_factor());
                let dimensions: [f32; 2] = window.inner_size().into();

                cursor_position = [
                    position_physical.x / dimensions[0],
                    position_physical.y / dimensions[1],
                ];
            }
            Event::WindowEvent {
                ...
                let uniform_buffer_subbuffer = {
                    let time_since_run = SystemTime::now()
                        .duration_since(start_time)
                        .expect("Time error");
                    let time_ms = time_since_run.as_millis();
