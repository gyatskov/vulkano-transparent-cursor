...
const float BORDER_EPSILON = 0.005;
const float PI = 3.1425;

// Transparent black
const vec4 COLOR_EMPTY  = vec4(0.0, 0.0, 0.0, 0.1);
// Red
const vec4 COLOR_INNER  = vec4(1.0, 0.0, 0.0, 0.9);
// Green
const vec4 COLOR_BORDER = vec4(0.0, 1.0, 0.0, 0.5);
// Green
const vec4 COLOR_CLICKED = vec4(0.0, 1.0, 0.0, 0.5);
...
{
    return sin(2.0 * PI * length(p))*circleDist(p,radius);
}
...
void main() {
    ...
    const vec2 p_n = (p / screenResolution * 2.0 - 1.0) * vec2(aspectRatio.x, 1.);
    ...
    const vec2 cursorCenter_n = (cursorPosition*2.0-1.0)*vec2(aspectRatio.x, 1.);
    const float cursorFreq_Hz = 0.5;
    const float cursorRadiusMax_n = 0.3;
    const float cursorRadius_n = cursorRadiusMax_n * (sin(2.0 * PI * cursorFreq_Hz * time_ms/1000.)+1.0)*0.5;
